...
DEFINE_bool(cm, false, "Use RDMA-CM to set up connections");
...
DEFINE_bool(odp, false, "on-demand paging");
...
DEFINE_int32(thread, 1, "The number of threads for clients to the same host");
...
DEFINE_int32(cuda_device_id, 0, "Cuda device id");
#endif
...
DEFINE_int32(recv_sge_batch_size, 1, "The sge_num for server to post recv requests");
...
DEFINE_bool(inline, false, "Inline when message size is less than max size");

DEFINE_bool(prefetch, false, "ODP Prefetch");
